...
export default function VoiceAgentsPage() {
  ...
  const refreshHistory = useCallback(() => {
    listSessions(20).then(setHistory).catch(() => {});
  }, []);

  const sendTurn = useCallback(async (audioBlob: Blob, sess: VoiceSession) => {
    setState("processing");
    try {
      const { text } = await transcribeAudio(audioBlob, sess.id);
      if (!text || text.length < 2) { setState("listening"); return; }
      ...
      setTranscript((prev) => [...prev, { role: "assistant", text: reply, ts: new Date().toISOString() }]);
  ...
  const setupVAD = useCallback((mediaStream: MediaStream, sess: VoiceSession) => {
          ...
          if (recorderRef.current?.state === "recording" && chunksRef.current.length > 0) {
            ...
            sendTurn(blob, sess).then(() => {
              if (recorderRef.current && audioCtxRef.current) {
                ...
                newRec.ondataavailable = (e) => { if (e.data.size > 0) chunksRef.current.push(e.data); };
  ...
  return (
    <div className="container py-6 space-y-6 page-enter">
      <header className="space-y-2">
        <h1 className="text-3xl font-heading font-bold tracking-tight nexus-gradient-text">Voice Agents</h1>
        <p className="text-muted-foreground">Converse por voz com seus agentes em tempo real — STT + LLM + TTS nativo do navegador.</p>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card><CardContent className="pt-6 flex items-center gap-3">
          <Mic className="h-5 w-5 text-primary" />
          <div><p className="text-xs text-muted-foreground">Sessões</p><p className="text-2xl font-bold tabular-nums">{history.length}</p></div>
        </CardContent></Card>
        <Card><CardContent className="pt-6 flex items-center gap-3">
          <Clock className="h-5 w-5 text-primary" />
          <div><p className="text-xs text-muted-foreground">Minutos totais</p><p className="text-2xl font-bold tabular-nums">{totalMinutes.toFixed(1)}</p></div>
        </CardContent></Card>
        <Card><CardContent className="pt-6 flex items-center gap-3">
          <DollarSign className="h-5 w-5 text-primary" />
          <div><p className="text-xs text-muted-foreground">Custo acumulado</p><p className="text-2xl font-bold tabular-nums">${totalCost.toFixed(3)}</p></div>
        </CardContent></Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between flex-wrap gap-3">
            <CardTitle className="flex items-center gap-2"><Bot className="h-5 w-5" /> Conversa Ativa</CardTitle>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <Select value={agentId} onValueChange={setAgentId}>
                  <SelectTrigger className="max-w-sm"><SelectValue placeholder="Selecione um agente (opcional)" /></SelectTrigger>
                  <SelectContent>
                    {agents.map((a) => <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <Button size="lg" variant="gradient" className="h-16 text-lg w-full md:w-auto" onClick={handleStart}>
            ...
            <div className="space-y-4">
              ...
              <ScrollArea className="h-[320px] rounded-lg border p-4 bg-secondary/20">
                <div className="space-y-3">
                  {transcript.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">Aguardando sua fala...</p>}
                  {transcript.map((t, i) => (
                    <div key={i} className={`flex ${t.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${t.role === "user" ? "bg-primary text-primary-foreground" : "bg-card border"}`}>
                        {t.text}
                      </div>
                    </div>
                  ))}
        ...
        <CardContent>
          {history.length === 0 ? (
            ...
            <div className="space-y-2">
              {history.map((h) => (
                <div key={h.id} className="flex items-center justify-between rounded-lg border px-4 py-3 hover:bg-secondary/30 transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <Badge variant={h.status === "ended" ? "secondary" : "default"} className="text-[10px]">{h.status}</Badge>
                      <span className="text-sm font-medium">
                        {(h.duration_ms / 1000).toFixed(1)}s · {h.transcript.length} turnos
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {new Date(h.started_at).toLocaleString("pt-BR")} · ${(h.cost_cents / 100).toFixed(4)}
                    </p>
                  </div>
                  <Button size="icon" variant="ghost" onClick={() => handleDelete(h.id)} aria-label="Excluir sessão">
