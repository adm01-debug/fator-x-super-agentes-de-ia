...
export default function OncallPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        logger.error('load oncall failed', e);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleAdd = async () => {
    ...
    if (!form.user_name || !form.starts_at || !form.ends_at) {
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Quem responde a incidentes em cada janela
          </p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div>
                <Label>Ordem de escalação</Label>
                <Input
                  type="number"
                  min={1}
                  value={form.escalation_order}
                  onChange={(e) => setForm({ ...form, escalation_order: Number(e.target.value) })}
                />
              </div>
              <div>
                <Label>Notas</Label>
              ...
              {current.map((c, i) => (
                <div key={i} className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    até {format(new Date(c.ends_at), "dd/MM 'às' HH:mm", { locale: ptBR })}
                  </span>
                </div>
              ))}
            </div>
