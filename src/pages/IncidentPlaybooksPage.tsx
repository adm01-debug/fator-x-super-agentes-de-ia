...
export default function IncidentPlaybooksPage() {
  ...
  const [form, setForm] = useState({
    ...
    cooldown_minutes: 5,
    actions: [
      { type: 'notify' as ActionType, config: { message: 'Incidente detectado' } },
    ] as PlaybookAction[],
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        logger.error('load playbooks failed', e);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setForm({
        ...
        trigger_type: 'manual',
        cooldown_minutes: 5,
        actions: [{ type: 'notify', config: { message: 'Incidente detectado' } }],
      });
      await refresh();
    } catch (e) {
  ...
  const applyTemplate = async (idx: number) => {
    ...
    try {
      ...
      toast.error('Falha ao instalar template', { description: String(e) });
    }
  };

  const handleToggle = async (id: string, enabled: boolean) => {
    try {
      await togglePlaybook(id, enabled);
      await refresh();
    } catch (e) {
  ...
  const handleTrigger = async (id: string) => {
    try {
      ...
      toast.error('Falha ao disparar', { description: String(e) });
    }
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Auto-remediação para reduzir MTTR de minutos para segundos
          </p>
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="Ex: Provider down → switch"
                />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v: TriggerType) => setForm({ ...form, trigger_type: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <CardContent className="py-4">
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch
                        checked={pb.enabled}
                        onCheckedChange={(v) => handleToggle(pb.id, v)}
                      />
                      <Button size="sm" variant="outline" onClick={() => handleTrigger(pb.id)}>
          ...
          {PLAYBOOK_TEMPLATES.map((t, i) => (
            <Card key={i} className="hover:border-primary/40 transition-colors">
              ...
              <CardContent className="space-y-3">
                ...
                <Button size="sm" className="w-full" onClick={() => applyTemplate(i)}>
                  Usar template
                </Button>
              </CardContent>
            </Card>
