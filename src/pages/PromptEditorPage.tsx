...
export default function PromptEditorPage() {
  ...
  const computeScore = useCallback((text: string) => {
    ...
    return {
      ...
      safety:
        lower.includes('segurança') || lower.includes('não revele') || lower.includes('regra'),
      examples:
        lower.includes('exemplo') || lower.includes('user:') || lower.includes('assistant:'),
      fallback:
        lower.includes('fallback') || lower.includes('escalar') || lower.includes('não souber'),
    };
  }, []);
  ...
  const scorePercent = Math.round(
    (scoreItems.filter(([, v]) => v).length / scoreItems.length) * 100,
  );
    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-[1400px] mx-auto animate-page-enter">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-secondary/50 border border-border/50">
          ...
          <TabsTrigger value="score" className="text-xs data-[state=active]:bg-background">
            Score
          </TabsTrigger>
        </TabsList>
          ...
          {versions.length === 0 && (
            <div className="nexus-card flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Escreva o primeiro prompt para este agente.
              </p>
            </div>
          )}
          <div className="nexus-card">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              {dirty && (
                <Badge
                  variant="outline"
                  className="text-[11px] text-nexus-amber border-nexus-amber/30"
                >
                  não salvo
                </Badge>
              )}
              ...
              {versions.map((v) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`h-8 w-8 rounded-lg flex items-center justify-center ${v.is_active ? 'bg-primary/15' : 'bg-secondary'}`}
                    >
                      <GitBranch
                        className={`h-4 w-4 ${v.is_active ? 'text-primary' : 'text-muted-foreground'}`}
                      />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {v.is_active && (
                          <Badge className="bg-primary/10 text-primary text-[11px]">ativa</Badge>
                        )}
                      </div>
              ...
              {diffVersion ? (
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setActiveTab('editor');
                        setDirty(true);
                        toast.info('Conteúdo copiado para o editor');
                      }}
                    >
                      Copiar para editor
                    </Button>
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Clique em uma versão para visualizar.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="nexus-card">
            <div className="flex items-center gap-4 mb-4">
              <div className="relative h-16 w-16">
                <svg viewBox="0 0 36 36" className="h-16 w-16 -rotate-90">
                  ...
                  <circle
                    ...
                    strokeWidth="3"
                    strokeDasharray={`${scorePercent * 0.975} 97.5`}
                    strokeLinecap="round"
                  />
                </svg>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Análise automática baseada no conteúdo atual
                </p>
              </div>
            </div>
