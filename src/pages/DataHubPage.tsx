...
import { ConnectionCard } from '@/components/datahub/ConnectionCard';
import { DEFAULT_CONNECTIONS, type ConnectionDef } from '@/components/datahub/connections.data';
import { EntityDetailPanel } from '@/components/datahub/EntityDetailPanel';
import { ENTITY_ICONS, getConnectionLabel } from '@/components/datahub/entityMeta';
...
export default function DataHubPage() {
  ...
  useEffect(() => {
    storeSelectEntity(selectedEntity);
  }, [selectedEntity, storeSelectEntity]);
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-[1400px] mx-auto animate-page-enter">
      ...
      <InfoHint title="Como funciona o DataHub?">
        ...
        e <strong>cross-database</strong>.
      </InfoHint>
              ...
              {filteredEntities.map((entity) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left nexus-card p-4 transition-all hover:border-primary/30 ${isSelected ? 'border-primary/50 ring-1 ring-primary/20 bg-primary/5' : ''}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`h-9 w-9 rounded-lg flex items-center justify-center ${isSelected ? 'bg-primary/20' : 'bg-primary/10'}`}
                        >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {(entity.secondary?.length ?? 0) > 0 && (
                          <Badge variant="secondary" className="text-[11px]">
                            +{entity.secondary?.length} joins
                          </Badge>
                        )}
                        {(entity.cross_db?.length ?? 0) > 0 && (
                          <Badge
                            variant="outline"
                            className="text-[11px] border-nexus-emerald/30 text-nexus-emerald"
                          >
                            {entity.cross_db?.length} cross-db
                          </Badge>
                        )}
                        <ArrowRight
