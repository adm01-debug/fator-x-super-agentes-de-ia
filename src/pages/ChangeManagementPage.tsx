...
export default function ChangeManagementPage() {
  ...
  const refreshApprovals = async (c: ChangeRequest) => {
    try {
      setApprovals(await listApprovals(c.id));
    } catch {
      /* noop */
    }
  };
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-[1400px] mx-auto animate-page-enter">
          ...
          <Dialog open={freezeOpen} onOpenChange={setFreezeOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" className="gap-2">
                <Snowflake className="h-4 w-4" /> Nova janela
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Bloqueia mudanças não-emergenciais durante o período
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFreezeForm({ ...freezeForm, name: e.target.value })}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      type="datetime-local"
                      value={freezeForm.starts_at}
                      onChange={(e) => setFreezeForm({ ...freezeForm, starts_at: e.target.value })}
                    />
                  </div>
                  <div>
                    <Label>Fim</Label>
                    <Input
                      type="datetime-local"
                      value={freezeForm.ends_at}
                      onChange={(e) => setFreezeForm({ ...freezeForm, ends_at: e.target.value })}
                    />
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Switch
                    checked={freezeForm.allow_emergency}
                    onCheckedChange={(v) => setFreezeForm({ ...freezeForm, allow_emergency: v })}
                  />
              ...
              <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Ex: Migrar DB para PG16"
                  />
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setForm({ ...form, change_type: v as ChangeType })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setForm({ ...form, risk_level: v as ChangeRiskLevel })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                ...
                <div>
                  <Label>Sistemas afetados (separados por vírgula)</Label>
                  <Input
                    value={form.affected_systems}
                    onChange={(e) => setForm({ ...form, affected_systems: e.target.value })}
                    placeholder="auth-service, billing-api"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    rows={3}
                    placeholder="Passos para reverter caso a mudança falhe..."
                  />
                </div>
                <div>
                  <Label>Validação pós-deploy</Label>
                  <Textarea
                    value={form.validation_steps}
                    onChange={(e) => setForm({ ...form, validation_steps: e.target.value })}
                    rows={3}
                    placeholder="Como confirmar que a mudança foi bem-sucedida..."
                  />
                </div>
              </div>
              <DialogFooter>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          icon={<AlertOctagon className="h-4 w-4 text-nexus-amber" />}
        ...
        <StatCard
          ...
          value={summary?.scheduled_7d ?? 0}
          loading={loading}
        />
        <StatCard
          icon={<Snowflake className="h-4 w-4 text-primary" />}
          label="Freezes ativos"
          value={summary?.in_freeze ?? 0}
          loading={loading}
          pulse={(summary?.in_freeze ?? 0) > 0}
        />
        <StatCard
          icon={<CheckCircle2 className="h-4 w-4 text-nexus-emerald" />}
          label="Taxa de sucesso (30d)"
          value={`${summary?.success_rate_30d ?? 0}%`}
          loading={loading}
        />
      </div>
      ...
      <Tabs defaultValue="changes">
        <TabsList>
          <TabsTrigger value="changes" className="gap-2">
            <GitPullRequest className="h-4 w-4" /> Mudanças
          </TabsTrigger>
          <TabsTrigger value="freezes" className="gap-2">
            <Snowflake className="h-4 w-4" /> Janelas de freeze
          </TabsTrigger>
        </TabsList>
                    ...
                    {filtered.map((c) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge variant="outline">{typeLabel(c.change_type)}</Badge>
                          </TableCell>
                          <TableCell>
                            <Badge variant="outline" className={riskVariant(c.risk_level)}>
                              {c.risk_level}
                            </Badge>
                          </TableCell>
                          <TableCell>
                    ...
                    {freezes.map((f) => {
                      ...
                      return (
                        <TableRow key={f.id}>
                          ...
                          <TableCell className="text-sm">
                            {new Date(f.starts_at).toLocaleString('pt-BR')} →{' '}
                            {new Date(f.ends_at).toLocaleString('pt-BR')}
                          </TableCell>
                          <TableCell>
                            {f.allow_emergency ? (
                              <Badge
                                variant="outline"
                                className="bg-nexus-amber/15 text-nexus-amber border-nexus-amber/30"
                              >
                                Permitidas
                              </Badge>
                            ) : (
            ...
            <>
              <SheetHeader>
                ...
                <SheetDescription className="flex gap-2 flex-wrap pt-2">
                  ...
                  <Badge variant="outline" className={riskVariant(selected.risk_level)}>
                    {selected.risk_level}
                  </Badge>
                  <Badge variant="outline" className={statusVariant(selected.status).className}>
              ...
              <Tabs defaultValue="details" className="mt-6">
                ...
                <TabsContent value="details" className="space-y-4 pt-4">
                  ...
                  {selected.scheduled_for && (
                    <Field
                      label="Agendado para"
                      value={new Date(selected.scheduled_for).toLocaleString('pt-BR')}
                    />
                  )}
                  {selected.rollback_plan && (
                    <Field label="Plano de rollback" value={selected.rollback_plan} mono />
                  )}
                  {selected.validation_steps && (
                    <Field label="Validação" value={selected.validation_steps} mono />
                  )}
                  {selected.post_mortem_url && (
                    <div>
                      <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">
                        Post-mortem
                      </p>
                      <a
                        href={selected.post_mortem_url}
                        target="_blank"
                        rel="noreferrer"
                  ...
                  {selected.status === 'pending_approval' && (
                    <Card>
                      <CardContent className="p-4 space-y-3">
                        ...
                        />
                        <div className="flex gap-2">
                          <Button onClick={() => handleDecide('approve')} className="flex-1">
                            Aprovar
                          </Button>
                          <Button
                ...
                <TabsContent value="execution" className="space-y-4 pt-4">
                  {selected.status === 'approved' && (
                    <Card>
                      <CardContent className="p-4">
                        <Button onClick={() => handleExecute(true)} className="w-full">
                          Iniciar execução
                        </Button>
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-base flex items-center gap-2">
                          <ShieldAlert className="h-4 w-4" /> Rollback
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-2">
                        <Input
                          value={rollbackUrl}
                          onChange={(e) => setRollbackUrl(e.target.value)}
                          placeholder="URL do post-mortem (obrigatório)"
                        />
                        <Button onClick={handleRollback} variant="outline" className="w-full">
                          Registrar rollback
                        </Button>
                      </CardContent>
                    </Card>
                  ...
                  {selected.executed_at && (
                    <Field
                      label="Executado em"
                      value={new Date(selected.executed_at).toLocaleString('pt-BR')}
                    />
                  )}
                  {selected.completed_at && (
                    <Field
                      label="Concluído em"
                      value={new Date(selected.completed_at).toLocaleString('pt-BR')}
                    />
                  )}
                </TabsContent>
              </Tabs>
...
function Field({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <p
        className={`text-sm whitespace-pre-wrap ${mono ? 'font-mono bg-muted/40 p-2 rounded' : ''}`}
      >
        {value}
      </p>
    </div>
