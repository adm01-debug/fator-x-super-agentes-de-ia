...
export default function SmolagentPage() {
  ...
  useEffect(() => {
    supabase.functions
      .invoke('smolagent-runtime', { body: { action: 'list_tools' } })
      .then(({ data }) => {
        if (data?.tools) setTools(data.tools.map((t: AnyData) => t.name));
      })
      .catch((err) => {
        logger.warn('Failed to fetch smolagent tools', {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-[1400px] mx-auto animate-page-enter">
      <PageHeader
        title="🤖 Smolagent Runtime"
        description="Runtime ReAct Agent — Pense, Aja, Observe, Repita"
        gradient={false}
      />
      <InfoHint title="O que é o Smolagent?">
        Runtime autônomo baseado no padrão ReAct. Recebe uma tarefa, pensa, executa ferramentas
        (busca web, edge functions, APIs), observa e repete até a resposta final.
      </InfoHint>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-lg border border-border p-4 space-y-4">
            ...
            <div className="space-y-2">
              <Label>Tarefa</Label>
              <Textarea
                placeholder="Ex: Pesquise sobre IA generativa e crie um resumo..."
                value={task}
            ...
            <div className="space-y-2">
              ...
              <Select value={model} onValueChange={setModel}>
                ...
                <SelectContent>
                  <SelectItem value="huggingface/Qwen/Qwen3-30B-A3B">Qwen3 30B (Free)</SelectItem>
                  <SelectItem value="huggingface/meta-llama/Llama-4-Scout-17B-16E-Instruct">
                    Llama 4 Scout 17B
                  </SelectItem>
                  <SelectItem value="huggingface/mistralai/Mistral-Small-24B-Instruct-2501">
                    Mistral Small 24B
                  </SelectItem>
                </SelectContent>
              </Select>
        ...
        <div className="lg:col-span-3 space-y-4" ref={resultRef}>
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-sm text-muted-foreground animate-pulse">Agente pensando...</p>
            </div>
          )}
          {result && !loading && (
            <>
                  ...
                  <div key={idx} className="rounded-lg border border-border overflow-hidden">
                    ...
                    {expandedSteps.has(idx) && (
                      <div className="border-t border-border p-3 bg-muted/20 space-y-2 text-xs">
                        {step.thought && (
                          <div>
                            <span className="font-semibold text-primary">💭 Thought:</span>
                        ...
                        {step.action_input && (
                          <div>
                            <span className="font-semibold text-accent-foreground">⚡ Input:</span>
                            <pre className="whitespace-pre-wrap mt-1 text-muted-foreground">
                              {step.action_input}
                            </pre>
                          </div>
          ...
          {!result && !loading && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-md">
                Descreva uma tarefa e o agente autônomo irá executá-la.
              </p>
            </div>
          )}
        </div>
