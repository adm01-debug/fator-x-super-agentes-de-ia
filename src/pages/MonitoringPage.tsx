...
const PIE_COLORS = [
  'hsl(var(--primary))',
  'hsl(var(--nexus-emerald, 142 71% 45%))',
  'hsl(var(--nexus-amber, 38 92% 50%))',
  'hsl(var(--nexus-cyan, 190 90% 50%))',
  'hsl(var(--destructive))',
];
...
export default function MonitoringPage() {
  ...
  const selected = filteredTraces.find((t) => t.id === selectedId) || filteredTraces[0];
  const stats = {
    total: filteredTraces.length,
    errors: filteredTraces.filter((t) => t.level === 'error' || t.level === 'critical').length,
    avgLatency: filteredTraces.length
      ? Math.round(
          filteredTraces.reduce((s: number, t) => s + (t.latency_ms || 0), 0) /
            filteredTraces.length,
        )
      : 0,
    totalCost: filteredTraces.reduce((s: number, t) => s + Number(t.cost_usd || 0), 0),
  };
  const levelCounts = filteredTraces.reduce<Record<string, number>>((acc, t) => {
  ...
  const latencyData = filteredTraces
    .slice(0, 20)
    .reverse()
    .map((t) => ({
      time: new Date(t.created_at).toLocaleTimeString('pt-BR', {
        hour: '2-digit',
        minute: '2-digit',
      }),
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-[1400px] mx-auto animate-page-enter">
      <PageHeader
        title="Monitoramento"
        description="Traces, sessões, alertas e observabilidade em tempo real"
      />
      <SystemHealthBanner />
      <div className="flex items-center gap-3 flex-wrap">
        <Select value={agentFilter} onValueChange={setAgentFilter}>
          <SelectTrigger className="w-[220px] bg-secondary/50 text-xs">
      ...
      <Tabs defaultValue="traces" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="sessions" className="gap-1.5">
            ...
            {sessions.length > 0 && (
              <Badge variant="secondary" className="text-[11px] h-4 px-1 ml-1">
                {sessions.length}
              </Badge>
            )}
          </TabsTrigger>
          <TabsTrigger value="alerts" className="gap-1.5">
            Alertas{' '}
            {unresolvedCount > 0 && (
              <Badge variant="destructive" className="text-[11px] h-4 px-1">
                {unresolvedCount}
              </Badge>
            )}
          </TabsTrigger>
          ...
          <TabsTrigger value="middleware" className="gap-1.5">
            <Layers className="h-3.5 w-3.5" /> Middleware
          </TabsTrigger>
          <TabsTrigger value="resilience" className="gap-1.5">
            <Shield className="h-3.5 w-3.5" /> Resiliência
          </TabsTrigger>
        </TabsList>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Traces aparecerão aqui quando agentes estiverem em produção.
              </p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="nexus-card text-center py-3">
                  ...
                  <p className="text-[11px] text-muted-foreground">Total de traces</p>
                </div>
                <div className="nexus-card text-center py-3">
                  ...
                  {filteredTraces.map((trace) => (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {trace.session_id || trace.id.slice(0, 8)}
                      </p>
                      <div className="flex items-center gap-3 mt-2 text-[11px] text-muted-foreground">
                        {trace.latency_ms && (
                          <span className="flex items-center gap-1">
                            <Clock className="h-3 w-3" /> {(trace.latency_ms / 1000).toFixed(1)}s
                          </span>
                        )}
                        {trace.cost_usd != null && (
                          <span className="flex items-center gap-1">
                            <DollarSign className="h-3 w-3" /> ${Number(trace.cost_usd).toFixed(3)}
                          </span>
                        )}
                        {trace.tokens_used && (
                          <span className="flex items-center gap-1">
                            <Wrench className="h-3 w-3" /> {String(trace.tokens_used)}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="lg:col-span-2 nexus-card">
                    ...
                    <div className="space-y-3">
                      {selected.input && (
                        <div>
                          ...
                          <pre className="text-[11px] text-muted-foreground bg-secondary/30 p-3 rounded-lg overflow-x-auto max-h-[200px]">
                    ...
                    <div className="mt-4 pt-4 border-t border-border/50 grid grid-cols-3 gap-4 text-center">
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">Latência</p>
                      </div>
                      <div>
                        <p className="text-lg font-heading font-bold text-foreground">
                          {selected.tokens_used?.toLocaleString() || '—'}
                        </p>
                      ...
                      <div>
                        <p className="text-lg font-heading font-bold text-foreground">
                          {selected.cost_usd != null
                            ? `$${Number(selected.cost_usd).toFixed(3)}`
                            : '—'}
                        </p>
                ...
                <div key={session.id} className="nexus-card">
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Agente: {agentName(session.agent_id)} • Início:{' '}
                        {session.started_at
                        ...
                        sessionTraces.map((st) => (
                          <div key={st.id} className="rounded-lg bg-secondary/20 p-2.5">
                            <div
                              ...
                              onClick={() =>
                                setExpandedTraceId(expandedTraceId === st.id ? null : st.id)
                              }
                              onKeyDown={(e) => {
                                if (e.key === 'Enter' || e.key === ' ') {
                                  e.preventDefault();
                            ...
                            >
                              ...
                              <span className="text-foreground truncate flex-1">
                                ...
                                {st.tokens_used ? `• ${st.tokens_used}t` : ''}
                              </span>
                              ...
                              <div className="mt-2 space-y-2">
                                {st.input ? (
                                  <div>
                                    ...
                                    <pre className="text-[11px] text-muted-foreground bg-secondary/30 p-2 rounded max-h-[120px] overflow-auto">
                                      {String(
          ...
          {loadingAlerts ? (
            ...
            <div className="space-y-3">
              {alerts.map((alert) => (
                ...
                >
                  <div
                    className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${alert.severity === 'critical' ? 'bg-destructive/10' : alert.severity === 'warning' ? 'bg-nexus-amber/10' : 'bg-primary/10'}`}
                  >
                    <Bell
                      className={`h-4 w-4 ${alert.severity === 'critical' ? 'text-destructive' : alert.severity === 'warning' ? 'text-nexus-amber' : 'text-primary'}`}
                    />
                  </div>
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {alert.is_resolved && (
                        <Badge variant="secondary" className="text-[11px] gap-1">
                          <CheckCircle className="h-2.5 w-2.5" /> Resolvido
                        </Badge>
                      )}
                    </div>
                    {alert.message && (
