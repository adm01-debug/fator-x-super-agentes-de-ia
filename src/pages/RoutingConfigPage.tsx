...
export function RoutingConfigPage() {
  ...
  const handleBulkToggle = async (source: string, enabled: boolean) => {
    try {
      ...
      toast.success(
        `${count} rota${count === 1 ? '' : 's'} ${enabled ? 'ativada' : 'desativada'}${count === 1 ? '' : 's'}`,
      );
      queryClient.invalidateQueries({ queryKey: ['agent_routing_all'] });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Erro');
      ...
      <header className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          <Badge variant="outline" className="text-xs">
            {routes.length} {t('routing.total_routes')}
          </Badge>
          ...
          {orphanRoutes > 0 && (
            <Badge
              variant="outline"
              className="text-xs border-nexus-amber/50 text-nexus-amber gap-1"
            >
        ...
        {summaries.map((s) => {
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-foreground">
                    {s.enabled} {t('routing.active')}
                  </span>
                </div>
                <div className="flex items-center gap-1">
                  <XCircle className="h-3 w-3 text-muted-foreground" />
                  <span className="text-muted-foreground">
                    {s.disabled} {t('routing.off')}
                  </span>
                </div>
              ...
              <div
                className="flex gap-1 mt-3 pt-3 border-t border-border/30"
              ...
              >
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleBulkToggle(s.source, true);
                  }}
                ...
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleBulkToggle(s.source, false);
                  }}
          ...
          <div className="max-h-[600px] overflow-y-auto">
            <table className="w-full text-xs">
              <thead className="border-b border-border/30 sticky top-0 bg-card z-10">
                <tr>
                  <th className="text-left px-3 py-2 font-medium text-[10px] uppercase tracking-wider text-muted-foreground">
                    {t('routing.source')}
                  </th>
                  <th className="text-left px-3 py-2 font-medium text-[10px] uppercase tracking-wider text-muted-foreground">
                    {t('routing.event')}
                  ...
                  <th className="text-center px-3 py-2 font-medium text-[10px] uppercase tracking-wider text-muted-foreground">
                    {t('routing.enabled')}
                  </th>
                  ...
                  return (
                    <tr key={r.id} className="border-b border-border/20 hover:bg-secondary/20">
                      ...
                      <td className="px-3 py-2 text-foreground">
                        {agent?.name ?? (
                          <span className="text-nexus-amber italic text-[10px]">
                            {t('routing.orphan_label')}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-3 py-2 text-center">
                        <Switch
                          ...
                          onCheckedChange={() => handleToggle(r.id, r.is_enabled)}
                        />
                      </td>
