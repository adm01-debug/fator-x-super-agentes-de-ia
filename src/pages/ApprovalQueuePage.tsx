  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-[1400px] mx-auto animate-page-enter">
      <PageHeader
        ...
        description="Human-in-the-Loop: workflows aguardando revisão humana"
      />
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Workflows com gates HITL aparecerão aqui quando precisarem de aprovação humana.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {pendingRuns.map((run) => {
            ...
            return (
              <div key={run.id} className={`nexus-card ${isSelected ? 'border-primary/30' : ''}`}>
                <div
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Etapa {run.current_step}/{run.total_steps} • Nodo:{' '}
                      {pending?.label || 'HITL Gate'}
                    </p>
                    {pending?.context && (
                      <p className="text-xs text-foreground/80 mt-1 max-w-xl">
                ...
                {isSelected && (
                  <div className="mt-4 pt-4 border-t border-border/30 space-y-3">
                    <Textarea
                      ...
                      placeholder="Feedback ou modificações (opcional)"
                      className="bg-secondary/50 text-xs min-h-[60px]"
                    />
                    <div className="flex gap-2">
                      <Button
                        onClick={() => handleApprove(String(run.id))}
                        disabled={processing}
                        className="nexus-gradient-bg text-primary-foreground gap-1.5 text-xs"
                      >
                        {processing ? (
                      ...
                      <Button
                        variant="destructive"
                        onClick={() => handleReject(String(run.id))}
                        disabled={processing}
