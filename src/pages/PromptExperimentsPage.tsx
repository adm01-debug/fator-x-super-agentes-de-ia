...
} from '@/services/promptExperimentService';
    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-[1400px] mx-auto animate-page-enter">
        ...
        <Card>
          <CardContent className="py-16 text-center space-y-3">
            ...
            <div className="text-sm text-muted-foreground">
              Nenhum experimento ainda. Crie seu primeiro A/B test.
            </div>
        ...
        <div className="grid gap-4">
          {experiments.map((exp) => (
            <ExperimentCard
              ...
              onDelete={() => {
                if (confirm('Remover experimento?')) remove.mutate(exp.id);
              }}
            />
          ))}
        </div>
      ...
      <Dialog open={!!selectedId} onOpenChange={(o) => !o && setSelectedId(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Detalhes do experimento</DialogTitle>
          </DialogHeader>
    ...
    <Card className="hover:shadow-lg transition-all cursor-pointer" onClick={onOpen}>
      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-xs">
          ...
          <Metric
            label="Split"
            value={`${experiment.traffic_split}/${100 - experiment.traffic_split}`}
          />
          <Metric label="Runs" value={totalRuns.toLocaleString('pt-BR')} />
          <Metric
            label="p-value"
            value={stats ? stats.p_value.toFixed(3) : '—'}
            highlight={stats?.significant}
          />
          <Metric
            label="Vencedor"
            value={
              experiment.winner
                ? experiment.winner.toUpperCase()
                : stats?.significant
                  ? `${stats.winner_candidate.toUpperCase()} ✓`
                  : '—'
            }
            highlight={!!experiment.winner}
          />
        </div>
      </CardContent>
    </Card>
...
function Metric({
  ...
  return (
    <div>
      ...
      <div className={`text-sm font-semibold mt-0.5 ${highlight ? 'text-nexus-green' : ''}`}>
        {value}
      </div>
    </div>
  );
...
function ExperimentDetail({ experimentId }: { experimentId: string }) {
  ...
  const simulate = useMutation({
    mutationFn: async () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        promises.push(
          promptExperimentService.recordRun({
            experiment_id: experimentId,
            variant,
            latency_ms: Math.floor(700 + Math.random() * 400 + (variant === 'b' ? 80 : 0)),
            cost_cents: Number(
              (0.4 + Math.random() * 0.3 + (variant === 'b' ? 0.1 : 0)).toFixed(4),
            ),
            quality_score: Number(
              (0.6 + Math.random() * 0.3 + (variant === 'b' ? 0.08 : 0)).toFixed(3),
            ),
  ...
  const guardrailsOk = useMemo(() => {
    ...
    const costInc =
      loser.avg_cost_cents > 0
        ? ((winner.avg_cost_cents - loser.avg_cost_cents) / loser.avg_cost_cents) * 100
        : 0;
    const latInc = winner.avg_latency_ms - loser.avg_latency_ms;
    return (
      costInc <= experiment.guardrails.max_cost_increase_pct &&
      ...
      <TabsContent value="overview" className="space-y-4">
            ...
            <CardContent className="p-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold">
                    Significância: p={stats.p_value.toFixed(4)}{' '}
                    {stats.significant ? '✓ significativo' : '— ainda não'}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    n={stats.variant_a.runs + stats.variant_b.runs} · z={stats.z_score.toFixed(2)} ·
                    winner candidato: {stats.winner_candidate.toUpperCase()}
                  </div>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                >
                  ...
                  Promover {stats.winner_candidate.toUpperCase()}
                </Button>
              </div>
            </CardContent>
          </Card>
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <VariantCard
              label={experiment.variant_a_label}
              variant="a"
              stats={stats.variant_a}
              winner={experiment.winner === 'a' || stats.winner_candidate === 'a'}
            />
            <VariantCard
              label={experiment.variant_b_label}
              variant="b"
              stats={stats.variant_b}
              winner={experiment.winner === 'b' || stats.winner_candidate === 'b'}
            />
          </div>
        )}
      </TabsContent>
                ...
                {runs.length === 0 ? (
                  ...
                  runs.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">{r.variant.toUpperCase()}</Badge>
                      </TableCell>
          ...
          <CardContent className="p-4 space-y-3 text-sm">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <b>{STATUS_LABEL[experiment.status]}</b>
              </div>
              <div>
                <span className="text-muted-foreground">Criado:</span>{' '}
                <b>{format(new Date(experiment.created_at), 'dd/MM/yyyy', { locale: ptBR })}</b>
              </div>
            </div>
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-3 gap-3 text-xs">
                <div>
                  <span className="text-muted-foreground">Custo máx:</span> +
                  {experiment.guardrails.max_cost_increase_pct}%
                </div>
                <div>
                  <span className="text-muted-foreground">Latência máx:</span> +
                  {experiment.guardrails.max_latency_increase_ms}ms
                </div>
                <div>
                  <span className="text-muted-foreground">Qualidade mín:</span>{' '}
                  {experiment.guardrails.min_quality}
                </div>
              </div>
            </div>
          </CardContent>
...
function VariantCard({
  ...
  label: string;
  variant: 'a' | 'b';
  stats: ExperimentStats['variant_a'];
  winner: boolean;
}) {
  return (
    <Card
      className={
        winner ? 'border-nexus-green/40 shadow-[0_0_20px_hsl(var(--nexus-green)/0.12)]' : ''
      }
    >
      <CardHeader className="pb-3">
      ...
      <CardContent className="space-y-3 text-xs">
        ...
        <div>
          <div className="flex justify-between mb-1">
            <span>Qualidade</span>
            <span className="font-semibold">{(stats.avg_quality * 100).toFixed(0)}%</span>
          </div>
