...
export const HF_ENV_VARS = {
  ...
  toggles: [
    'ENABLE_ML_INJECTION_CHECK',
    'ENABLE_HF_RERANKER',
    'ENABLE_HF_EMBEDDINGS',
    'ENABLE_AUTO_CLASSIFY',
    'ENABLE_TOXICITY_CHECK',
...
const getDenoEnv = (key: string): string | null => {
  try {
    ...
    logger.error('Operation failed:', err); /* browser environment */
  }
  return null;
};

export function getEmbeddingsUrl(hfToken: string): {
  url: string;
  headers: Record<string, string>;
} {
  const teiEndpoint = getDenoEnv('HF_TEI_ENDPOINT');
  if (teiEndpoint)
    return { url: `${teiEndpoint}/embed`, headers: { 'Content-Type': 'application/json' } };
  return {
    url: `${HF_INFERENCE_URL}/${HF_MODELS_RAG.embeddings.id}`,
...
export function getRerankerUrl(hfToken: string): { url: string; headers: Record<string, string> } {
  ...
  if (teiEndpoint)
    return { url: `${teiEndpoint}/rerank`, headers: { 'Content-Type': 'application/json' } };
  return {
    url: `${HF_INFERENCE_URL}/${HF_MODELS_RAG.reranker.id}`,
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${hfToken}` },
  };
}
