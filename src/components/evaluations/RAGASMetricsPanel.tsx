...
import {
  ...
  type RAGASResult,
  type EvalTestCase,
} from '@/services/evalEngineService';
...
const METRIC_META: Record<string, { label: string; description: string; color: string }> = {
  faithfulness: {
    label: 'Factualidade',
    description: 'Resposta baseada nos contextos?',
    color: 'text-nexus-blue',
  },
  answer_relevancy: {
    label: 'Relevância',
    description: 'Resposta relevante à pergunta?',
    color: 'text-nexus-emerald',
  },
  context_precision: {
    label: 'Precisão de Contexto',
    description: 'Contextos relevantes no topo?',
    color: 'text-nexus-purple',
  },
  context_recall: {
    label: 'Recall de Contexto',
    description: 'Informação necessária coberta?',
    color: 'text-nexus-amber',
  },
  answer_correctness: {
    label: 'Corretude',
    description: 'Resposta factualmente correta?',
    color: 'text-primary',
  },
};
...
export function RAGASMetricsPanel() {
  ...
  const handleRunRAGAS = async () => {
    ...
    const testCases: EvalTestCase[] = testInput
      .split('\n')
      .filter(Boolean)
      .map((line) => ({
        query: line.trim(),
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">
            Avaliação de qualidade RAG (Faithfulness, Relevancy, Precision, Recall)
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="space-y-2">
          <span className="text-[11px] font-medium text-muted-foreground">
            Perguntas de teste (1 por linha)
          </span>
          <Textarea
      ...
      {result && (
        <div className="space-y-4 animate-fade-in">
          <div className="flex items-center gap-3">
            <div
              className={`text-2xl font-black ${result.overall_score >= 0.7 ? 'text-nexus-emerald' : result.overall_score >= 0.4 ? 'text-nexus-amber' : 'text-destructive'}`}
            >
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                {result.sample_count} amostra(s) avaliada(s)
              </p>
            </div>
          ...
          <div className="space-y-3">
            {Object.entries(METRIC_META).map(([key, meta]) => (
              <MetricBar
                key={key}
                value={(result as Record<string, number>)[key] ?? 0}
                label={meta.label}
                color={meta.color}
              />
            ))}
