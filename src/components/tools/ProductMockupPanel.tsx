/**
 * Nexus Agents Studio — Product Mockup Panel
 *
 * Promo Brindes-specific tool. Wraps the product-mockup Edge Function
 * with 4 actions in tabs: Mockup, Upscale, Inpaint, Segment.
 *
 * Used in ToolsPage as a high-ROI feature for the merchandise workflow.
 */
...
function MockupTab() {
  ...
  const handleGenerate = async () => {
    if (!productFile) {
      toast.error("Selecione uma imagem de produto");
      return;
    }
    setLoading(true);
    try {
          ...
          {steps.map((s, i) => (
            <div key={i} className="flex items-center gap-2">
              <Badge
                ...
                style={{
                  borderColor:
                    s.status === "ok" ? "#6BCB77" : s.status === "error" ? "#FF6B6B" : "#FFD93D",
                  color:
                    s.status === "ok" ? "#6BCB77" : s.status === "error" ? "#FF6B6B" : "#FFD93D",
                }}
              >
...
function InpaintTab() {
  ...
  const handleInpaint = async () => {
    if (!imageFile || !maskFile || !prompt.trim()) {
      return toast.error("Imagem, máscara e prompt são obrigatórios");
    }
    setLoading(true);
    try {
      const [imageBase64, maskBase64] = await Promise.all([
        imageToBase64(imageFile),
        imageToBase64(maskFile),
      ]);
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <FilePicker
          id="inpaint-mask"
          label="Máscara (PNG branco/preto)"
          onFile={(f) => {
            setMaskFile(f);
