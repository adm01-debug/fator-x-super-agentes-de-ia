...
const pages = [
  ...
  { name: 'Oráculo', href: '/oracle', icon: Sparkles, keywords: 'oracle conselho' },
  {
    ...
    href: '/knowledge',
    icon: BookOpen,
    keywords: 'documentos rag knowledge',
  },
  ...
  { name: 'Workflows', href: '/workflows', icon: GitBranch, keywords: 'automação fluxo' },
  {
    ...
    href: '/evaluations',
    icon: FlaskConical,
    keywords: 'testes avaliação evaluations',
  },
  {
    name: 'Implantações',
    href: '/deployments',
    icon: Rocket,
    keywords: 'deploy produção deployments',
  },
  ...
  const { data: agents = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabaseExternal
        .from('agents')
        .select('id, name, model, persona, avatar_emoji, status')
          ...
          <>
            ...
            <CommandGroup heading="Agentes">
              {agents.map((agent) => (
                <CommandItem
                  key={agent.id}
                  onSelect={() => go(`/builder/${agent.id}`)}
                  className="gap-2"
                  keywords={[agent.persona || '', agent.model || '', agent.name]}
                >
            ...
            <CommandGroup heading="Bases de Conhecimento">
              {knowledgeBases.map((kb) => (
                <CommandItem
                  ...
                  className="gap-2"
                  keywords={[kb.name, 'rag', 'documento', 'knowledge']}
                >
            ...
            <CommandGroup heading="Workflows">
              {workflows.map((wf) => (
                <CommandItem
                  ...
                  className="gap-2"
                  keywords={[wf.name, 'automação', 'fluxo', 'workflow']}
                >
                  <GitBranch className="h-4 w-4 text-nexus-cyan" />
                  <span>{wf.name}</span>
                  <span
                    className={`ml-auto text-[11px] ${wf.status === 'active' ? 'text-nexus-emerald' : 'text-muted-foreground'}`}
                  >
                    {wf.status}
                  </span>
                </CommandItem>
            ...
            <CommandGroup heading="Ferramentas">
              {tools.map((tool) => (
                <CommandItem
                  ...
                  onSelect={() => go('/tools')}
                  className="gap-2"
                  keywords={[tool.name, tool.type, 'ferramenta', 'integração']}
                >
