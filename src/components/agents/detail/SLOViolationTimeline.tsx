...
export function SLOViolationTimeline({ data, daily, onDayClick }: Props) {
  const maxSeverity = Math.max(
    1,
    ...data.map((d) => d.p95Violations + d.p99Violations * 2 + d.errors * 2),
  );
      ...
      <div role="list" className="space-y-1.5">
        {data.map((day) => {
          ...
          const status: 'healthy' | 'warning' | 'critical' =
            day.errors > 0 || day.p99Violations > 0
              ? 'critical'
              : day.p95Violations > 0
                ? 'warning'
                : 'healthy';
              ...
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  <span className="text-[11px] tabular-nums text-muted-foreground min-w-[80px] text-right">
                    {totalViolations === 0
                      ? 'sem violações'
                      : `${totalViolations} violaç${totalViolations === 1 ? 'ão' : 'ões'}`}
                  </span>
                </div>
              </TooltipTrigger>
              <TooltipContent side="top" className="text-xs">
                <div className="space-y-0.5">
                  <p className="font-semibold">
                    {day.label} · {day.total} trace{day.total !== 1 ? 's' : ''}
                  </p>
