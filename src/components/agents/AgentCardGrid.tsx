...
import {
  Star, Copy, Wand2, Download, ArrowRight, Loader2,
} from "lucide-react";
...
export function AgentCardGrid({
  agents, favorites, selectedIds, selectionMode, cloning,
      ...
      {agents.map((agent) => {
        ...
        return (
          <div
            ...
            onClick={() => selectionMode ? onToggleSelect({ stopPropagation: () => {} } as React.MouseEvent, agent.id) : onNavigate(`/builder/${agent.id}`)}
          >
            <div className={`absolute top-3 left-3 z-10 transition-opacity ${selectionMode || 'opacity-0 group-hover:opacity-100'}`}
            ...
            <div className={`flex items-start justify-between mb-3 ${selectionMode ? 'ml-7' : ''}`}>
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={(e) => onExport(e, agent)}
                  ...
                  aria-label="Exportar agente" title="Exportar JSON">
                  <Download className="h-3.5 w-3.5" />
                </button>
                <button onClick={(e) => onAutoTag(e, agent)}
                  ...
                  aria-label="Auto-tag com IA" title="Auto-tag">
                  <Wand2 className="h-3.5 w-3.5" />
                </button>
                <button onClick={(e) => onClone(e, agent)}
                  ...
                  aria-label="Clonar agente" disabled={cloning === agent.id}>
                  {cloning === agent.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Copy className="h-3.5 w-3.5" />}
                </button>
                <button onClick={(e) => onToggleFav(e, agent.id)}
                  className={`h-7 w-7 rounded-md flex items-center justify-center transition-all ${
                    isFav ? 'text-nexus-amber bg-nexus-amber/10' : 'text-muted-foreground/30 hover:text-nexus-amber hover:bg-nexus-amber/10 opacity-0 group-hover:opacity-100'
            ...
            <p className="text-xs text-muted-foreground line-clamp-2 mb-3 flex-1">
              {agent.mission || (config as Record<string, unknown>).description as string || "Sem descrição configurada"}
            </p>
            ...
            <div className="flex items-center justify-between mt-auto pt-2 border-t border-border/20">
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {(agent.tags ?? []).slice(0, 3).map(tag => (
                  <Badge key={tag} variant="outline" className="text-[11px] h-5 border-border/40 text-muted-foreground font-normal">{tag}</Badge>
                ))}
                {(agent.tags ?? []).length > 3 && (
