...
const EXAMPLES: Record<string, string> = {
  entities:
    'Acabamos de fechar contrato com a Promo Brindes via João Silva (Diretor Comercial) para entrega de 5.000 canetas personalizadas.',
  facts: 'Em 2024, a empresa cresceu 32% em receita, atingindo R$ 8,2 milhões com margem de 18%.',
  rules:
    'Pedidos acima de R$ 50.000 precisam de aprovação do diretor financeiro. Pagamentos só são liberados após confirmação fiscal.',
  contacts:
    'Maria Santos, gerente de compras, maria.santos@empresa.com.br, (11) 98765-4321, ramal 2341',
};
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-[11px] text-muted-foreground mt-0.5">
          Cole texto e extraia entidades, fatos, regras, contatos, datas, valores, tarefas ou riscos
          automaticamente
        </p>
      </div>
      ...
      <div className="nexus-card space-y-4">
        <div className="flex gap-3 items-end flex-wrap">
          <div className="flex-1 min-w-[220px] space-y-1.5">
            <span className="text-[11px] text-muted-foreground font-medium">Tipo de extração</span>
          ...
          <div className="p-4 rounded-lg bg-secondary/30 border border-border/30 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-foreground">
                Resultado (
                {EXTRACT_TYPES.find((t) => t.value === extractType)?.label ?? extractType})
              </p>
              {costUsd != null && (
