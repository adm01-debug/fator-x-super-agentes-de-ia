...
export function AlertRulesPanel() {
  ...
  const handleCreateRule = async () => {
    const result = alertRuleSchema.safeParse({
      ...
      metric: ruleMetric,
      operator: ruleOp,
      threshold: parseFloat(ruleThreshold) || 0,
  ...
  return (
    <div className="nexus-card mt-6">
      ...
      <p className="text-xs text-muted-foreground mb-4">
        Crie regras que disparam alertas quando métricas ultrapassam limites.
      </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-2 mb-4 items-end">
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded-lg border border-border bg-secondary/50 px-2 py-1.5 text-xs"
          >
            <option value="cost_usd">Custo (USD)</option>
            <option value="latency_ms">Latência (ms)</option>
            <option value="tokens_used">Tokens</option>
            <option value="error_count">Erros</option>
          </select>
        </div>
        <div>
          <span className="text-[11px] text-muted-foreground">Operador</span>
          <select
            value={ruleOp}
        ...
        <Button
          ...
          disabled={creating}
          className="nexus-gradient-bg text-primary-foreground text-xs"
        >
      ...
      {rules.length === 0 ? (
        <p className="text-xs text-muted-foreground text-center py-4">
          Nenhuma regra criada. Alertas de budget são automáticos via trigger.
        </p>
      ) : (
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => handleToggleRule(String(rule.id), e.target.checked)}
                  className="rounded"
                />
                <div>
                  <span className="font-medium text-foreground">{String(rule.name)}</span>
                  <span className="text-muted-foreground ml-2">
                    {String(rule.metric)} {String(rule.operator)} {String(rule.threshold)}
                  </span>
                </div>
                <Badge
