...
function ToggleField({ label, description, checked, onChange, icon: Icon }: {
  label: string; description?: string; checked: boolean; onChange: (v: boolean) => void; icon?: typeof Shield;
}) {
  return (
    <div className="flex items-center justify-between p-3 rounded-lg bg-background border border-border cursor-pointer hover:border-primary/30 transition-colors" onClick={() => onChange(!checked)}>
  ...
  return (
    <Card className="bg-card border-border">
      <CardContent className="pt-4 space-y-3">
        <ToggleField label="Acesso à Rede" description="Permitir que o código acesse a internet" checked={config.networkEnabled} onChange={(v) => onUpdate({ networkEnabled: v })} icon={config.networkEnabled ? Wifi : WifiOff} />
        <ToggleField label="Escrita em Disco" description="Permitir que o código crie/edite arquivos no workspace" checked={config.writeAccess} onChange={(v) => onUpdate({ writeAccess: v })} icon={HardDrive} />
        <ToggleField label="Acesso ao Shell" description="Permitir execução de comandos shell (bash/sh)" checked={config.shellAccess} onChange={(v) => onUpdate({ shellAccess: v })} icon={Terminal} />
        ...
        <div className="p-3 rounded-lg bg-background border border-border">
          ...
          <div className="space-y-1 text-xs">
            ...
            <div className="flex items-center justify-between"><span className="text-muted-foreground">Rede</span>
              <span className={config.networkEnabled ? 'text-nexus-amber' : 'text-nexus-emerald'}>{config.networkEnabled ? `Habilitada (${config.allowedDomains.length} domínios)` : 'Bloqueada'}</span>
            </div>
            <div className="flex items-center justify-between"><span className="text-muted-foreground">Shell</span>
              <span className={config.shellAccess ? 'text-destructive' : 'text-nexus-emerald'}>{config.shellAccess ? 'Habilitado ⚠️' : 'Bloqueado'}</span>
            </div>
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/30">
            ...
            <div>
              ...
              <p className="text-xs text-destructive/80 mt-1">
                {config.runtime === 'local' ? 'Runtime local não tem isolamento. Use apenas em ambiente de desenvolvimento.' : 'Acesso ao shell permite execução arbitrária de comandos. Use com cautela.'}
              </p>
            </div>
          </div>
        )}
