...
export function OracleHistory(_props: OracleHistoryProps) {
  ...
  if (loading) {
    return <div className="nexus-card text-center py-8 text-sm text-muted-foreground">Carregando histórico...</div>;
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="ghost" onClick={() => setShowFilters(!showFilters)} className="text-xs gap-1">
          ...
          <div className="overflow-hidden">
            <div className="p-3 rounded-lg bg-secondary/30 border border-border/30 grid grid-cols-3 gap-3">
              <Select value={filters.mode || '__all'} onValueChange={v => setFilters(f => ({ ...f, mode: v === '__all' ? undefined : v as OracleMode }))}>
                ...
                <SelectContent>
                  <SelectItem value="__all" className="text-xs">Todos os modos</SelectItem>
                  {Object.entries(ORACLE_MODES).map(([k, v]) => (
                    <SelectItem key={k} value={k} className="text-xs">{v.icon} {v.label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <Select value={filters.presetId || '__all'} onValueChange={v => setFilters(f => ({ ...f, presetId: v === '__all' ? undefined : v }))}>
                <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Preset" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="__all" className="text-xs">Todos os presets</SelectItem>
                  {ORACLE_PRESETS.map(p => (
                    <SelectItem key={p.id} value={p.id} className="text-xs">{p.icon} {p.name.replace(/^[^\s]+\s/, '')}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
      ...
      {/* Entries */}
      {entries.length === 0 ? (
        <div className="nexus-card text-center py-8 text-sm text-muted-foreground">Nenhuma consulta no histórico</div>
      ) : (
              ...
              <div key={entry.id} className="nexus-card p-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setExpandedId(expandedId === entry.id ? null : entry.id)}>
                      <Eye className="h-3.5 w-3.5" />
                    </Button>
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => handleExportMd(entry)}>
