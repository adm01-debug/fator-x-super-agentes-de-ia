...
const MODELS = [
  {
    ...
    description: 'Anthropic · 200K ctx · $15/$75 por 1M',
    badge: <NexusBadge color="orange">Premium</NexusBadge>,
    accentColor: 'hsl(var(--nexus-gold))',
  },
  {
    id: 'claude-sonnet-4.6',
    icon: '🔵',
    title: 'Claude Sonnet 4.6',
    description: 'Anthropic · 200K ctx · $3/$15 por 1M',
    badge: <NexusBadge color="blue">Recomendado</NexusBadge>,
    accentColor: 'hsl(var(--nexus-blue))',
  },
  {
    id: 'claude-haiku-4.5',
    icon: '🟢',
    title: 'Claude Haiku 4.5',
    description: 'Anthropic · 200K ctx · $0.25/$1.25 por 1M',
    badge: <NexusBadge color="green">Econômico</NexusBadge>,
    accentColor: 'hsl(var(--nexus-green))',
  },
  {
    id: 'gpt-4o',
    icon: '🔴',
    title: 'GPT-4o',
    description: 'OpenAI · 128K ctx · $2.50/$10 por 1M',
    badge: <NexusBadge color="red">Premium</NexusBadge>,
    accentColor: 'hsl(var(--nexus-red))',
  },
  {
    id: 'gemini-2.5-pro',
    icon: '🟡',
    title: 'Gemini 2.5 Pro',
    description: 'Google · 1M ctx · $1.25/$5 por 1M',
    badge: <NexusBadge color="yellow">Premium</NexusBadge>,
    accentColor: 'hsl(var(--nexus-yellow))',
  },
  {
    id: 'llama-4',
    icon: '🟣',
    title: 'Llama 4',
    description: 'Meta (Open) · 128K ctx · Custo infra',
    badge: <NexusBadge color="purple">Open Source</NexusBadge>,
    accentColor: 'hsl(var(--nexus-purple))',
  },
  {
    id: 'huggingface/Qwen/Qwen3-30B-A3B',
    icon: '🤗',
    title: 'Qwen3 30B',
    description: 'HuggingFace · MoE · $0.15/$0.30 por 1M',
    badge: <NexusBadge color="yellow">HF Free</NexusBadge>,
  ...
  },
  {
    ...
    description: 'HuggingFace · 24B · $0.10/$0.30 por 1M',
    badge: <NexusBadge color="yellow">HF Free</NexusBadge>,
    accentColor: 'hsl(45, 90%, 55%)',
  },
  {
    ...
    icon: '🤗',
    title: 'Llama 4 Scout',
    description: 'HuggingFace · 17B MoE · $0.17/$0.40 por 1M',
    badge: <NexusBadge color="yellow">HF Free</NexusBadge>,
    accentColor: 'hsl(45, 90%, 55%)',
  },
];
...
const REASONING_OPTIONS: {
...
}[] = [
  {
    id: 'react',
    name: 'ReAct',
    description: 'Raciocina, age, observa e itera. Padrão mais versátil.',
    recommended: true,
  },
  {
    id: 'cot',
    name: 'Chain-of-Thought',
    description: 'Passo-a-passo explícito antes de responder.',
  },
  {
    id: 'tot',
    name: 'Tree-of-Thought',
    description: 'Múltiplos caminhos de raciocínio em paralelo.',
  },
  {
    id: 'reflection',
    name: 'Reflection',
    description: 'Revisa e critica a própria resposta antes de entregar.',
  },
  {
    id: 'plan_execute',
    name: 'Plan & Execute',
    description: 'Cria plano completo, depois executa etapa por etapa.',
  },
  {
    id: 'smolagent',
    name: '🤗 SmolagentRT',
    description:
      'Agente autônomo com 13 ferramentas HF (busca, imagem, áudio, banco, etc). Executa ações reais.',
  },
];
  ...
  const handleAutoRoute = async () => {
    ...
    try {
      ...
      const matchingModel = MODELS.find(
        (m) => result.recommended_model.includes(m.id) || m.id.includes(result.recommended_model),
      );
      if (matchingModel) {
    ...
    <div className="space-y-10">
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          <SectionTitle
            icon="🧠"
            title="Modelo Principal (LLM)"
            subtitle="Selecione o modelo de linguagem que alimentará o cérebro do agente."
          />
          <Button
            variant="outline"
            size="sm"
          ...
          <div className="mb-4 p-3 rounded-lg border border-primary/20 bg-primary/5 animate-fade-in">
            <div className="flex items-center gap-2 flex-wrap">
              <Badge variant="outline" className="text-[10px]">
                Complexidade: {routeResult.complexity.level} (
                {(routeResult.complexity.score * 100).toFixed(0)}%)
              </Badge>
              <Badge variant="outline" className="text-[10px]">
                Tier: {routeResult.tier}
              </Badge>
              <Badge variant="outline" className="text-[10px]">
                ~${routeResult.estimated_cost_per_query.toFixed(4)}/query
              </Badge>
            </div>
            {routeResult.alternatives.length > 0 && (
              <p className="text-[11px] text-muted-foreground mt-1.5">
                Alternativas:{' '}
                {routeResult.alternatives.map((a) => `${a.model} (${a.tier})`).join(', ')}
              </p>
            )}
          </div>
        )}
      ...
      <section>
        <SectionTitle
          ...
          subtitle="Modelo alternativo caso o principal falhe ou atinja rate-limit."
        />
        <ToggleField
      ...
      <section>
        <SectionTitle
          icon="💡"
          title="Padrão de Raciocínio"
          subtitle="Como o agente estrutura seu pensamento antes de responder."
        />
        <RadioGroup
        ...
        >
          {REASONING_OPTIONS.map((opt) => (
            ...
            >
              <RadioGroupItem id={`reasoning-${opt.id}`} value={opt.id} className="mt-0.5" />
      ...
      <section>
        <SectionTitle
          ...
          subtitle="Ajuste fino do comportamento de geração."
        />
      ...
      <section>
        <SectionTitle
          ...
          subtitle="Skills carregadas dinamicamente no contexto do LLM para economizar tokens."
        />
        <div className="rounded-lg border border-border/50 bg-secondary/20 p-4">
          ...
          <p className="text-[11px] text-muted-foreground">
            O sistema carrega apenas as skills relevantes para cada tarefa, otimizando uso de tokens
            e qualidade da resposta. Skills são selecionadas por matching semântico com a task
            description.
          </p>
          {skills.length > 0 && (
