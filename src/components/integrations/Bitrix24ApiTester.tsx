/**
 * Bitrix24 API Tester — Test bitrix24-api Edge Function calls.
 */
...
const API_METHODS = [
  { value: 'crm.deal.list', label: 'Listar Negocios', category: 'CRM' },
  { value: 'crm.contact.list', label: 'Listar Contatos', category: 'CRM' },
  { value: 'crm.company.list', label: 'Listar Empresas', category: 'CRM' },
  { value: 'crm.lead.list', label: 'Listar Leads', category: 'CRM' },
  { value: 'crm.product.list', label: 'Listar Produtos', category: 'CRM' },
  { value: 'user.current', label: 'Usuario Atual', category: 'Sistema' },
  { value: 'app.info', label: 'Info do App', category: 'Sistema' },
];
...
export function Bitrix24ApiTester() {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const data = await invokeBitrix24Api({ method, params: parsedParams });
      setResult(data);
      toast.success('Chamada Bitrix24 executada');
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : 'Erro ao chamar Bitrix24 API');
    } finally {
      setTesting(false);
    }
  };
  ...
  return (
    <div className="nexus-card">
      ...
      <p className="text-xs text-muted-foreground mb-3">Teste chamadas diretas a API REST do Bitrix24 via proxy seguro.</p>
      ...
      <div className="grid md:grid-cols-3 gap-3 mb-3">
            ...
            <SelectContent>
              {API_METHODS.map(m => (
                <SelectItem key={m.value} value={m.value}>
                  ...
                  <Badge variant="outline" className="text-[8px] ml-1">{m.category}</Badge>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <div className="md:col-span-2">
          <label className="text-[11px] text-muted-foreground">Parametros (JSON opcional)</label>
          <Input
            value={params}
            onChange={e => setParams(e.target.value)}
            placeholder='{"filter": {"STATUS_ID": "NEW"}, "select": ["ID", "TITLE"]}'
      ...
      {result && (
        <div className="mt-3 p-3 rounded-lg bg-secondary/30 border border-border/30">
          ...
          <pre className="text-[11px] text-muted-foreground overflow-auto max-h-[200px] font-mono">
