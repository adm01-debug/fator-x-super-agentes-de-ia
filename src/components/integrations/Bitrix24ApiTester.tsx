...
export function Bitrix24ApiTester() {
  ...
  const handleTest = async () => {
    ...
    setResult(null);
    try {
      const data = await invokeBitrix24Api({
        method,
      ...
      toast.error(e instanceof Error ? e.message : 'Erro ao chamar Bitrix24');
    } finally {
      setTesting(false);
    }
  };
  ...
  return (
    <div className="nexus-card">
      ...
      <p className="text-xs text-muted-foreground mb-3">
        Teste chamadas a API REST do Bitrix24 via proxy seguro.
      </p>
      <div className="grid md:grid-cols-3 gap-3 mb-3">
        <div>
          <span className="text-[11px] text-muted-foreground">Metodo</span>
        ...
        <div className="md:col-span-2">
          <span className="text-[11px] text-muted-foreground">Parametros (JSON)</span>
          <Input
            value={params}
            onChange={(e) => setParams(e.target.value)}
            placeholder='{"filter": {"STATUS_ID": "NEW"}}'
