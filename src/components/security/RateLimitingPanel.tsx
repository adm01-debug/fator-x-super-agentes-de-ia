  ...
  return (
    <div className="nexus-card">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button variant="ghost" size="sm" className="h-7 text-xs gap-1" onClick={loadStats} disabled={loading}>
        ...
        {limits.map(l => {
          ...
          return (
            <div key={l.name} className="p-3 rounded-lg bg-secondary/20 border border-border/30">
              ...
              {isHigh && <p className="text-[10px] text-nexus-amber mt-1">Proximo do limite</p>}
            </div>
          );
        })}
      </div>
