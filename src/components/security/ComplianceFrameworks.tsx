...
export function ComplianceFrameworks() {
  const [frameworks, setFrameworks] = useState<
    Array<{ name: string; status: string; coverage: number }>
  >([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        postureScore =
          p.length > 0
            ? Math.round((p.filter((c) => c.status === 'pass').length / p.length) * 100)
            : 0;
      } catch {
        // Falha silenciosa: métrica fica em 0.
      }
      try {
        const g = await listGuardrailPolicies();
        hasGuardrails = g.some((x) => x.is_enabled === true);
      } catch {
        // Falha silenciosa: considera sem guardrails.
      }
      try {
        const l = await getAuditLog({ limit: 1 });
        hasLogs = l.length > 0;
      } catch {
        // Falha silenciosa: considera sem logs.
      }
    ...
    <div className="nexus-card">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {frameworks.map((fw) => (
          <div
            key={fw.name}
            className="p-3 rounded-lg bg-secondary/30 border border-border/30 text-center"
          >
            <p className="text-sm font-heading font-bold text-foreground">{fw.name}</p>
            <div className="mt-2 h-1.5 rounded-full bg-secondary overflow-hidden">
              <div
                className="h-full rounded-full nexus-gradient-bg transition-all"
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <Badge
                variant="outline"
                className={`text-[11px] ${fw.status === 'compliant' ? 'border-nexus-emerald/30 text-nexus-emerald' : fw.status === 'partial' ? 'border-nexus-amber/30 text-nexus-amber' : 'border-muted-foreground/30 text-muted-foreground'}`}
              >
                {fw.status === 'compliant'
                  ? 'Conforme'
                  : fw.status === 'partial'
