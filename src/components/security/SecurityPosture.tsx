...
const iconMap: Record<string, typeof Lock> = { tls: Lock, api_keys: Key, pii: Eye, jailbreak: UserX, guardrails: ShieldAlert, audit: ShieldCheck };

export function SecurityPosture() {
  const [checks, setChecks] = useState<Array<{ id: string; title: string; desc: string; status: 'pass' | 'warn' }>>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getSecurityPosture().then(setChecks).catch(() => setChecks([])).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="nexus-card">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`text-xs font-bold px-2 py-0.5 rounded-full ${score >= 80 ? 'bg-nexus-emerald/10 text-nexus-emerald' : 'bg-nexus-amber/10 text-nexus-amber'}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {checks.map((check) => {
          ...
          return (
            <div key={check.id} className="p-3 rounded-lg bg-secondary/30 border border-border/30 flex items-start gap-3">
