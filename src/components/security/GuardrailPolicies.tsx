...
export function GuardrailPolicies() {
  ...
  const handleToggle = async (id: string, enabled: boolean) => {
    try {
      await toggleGuardrailPolicy(id, enabled);
    } catch (e: unknown) {
    ...
    <div className="nexus-card">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-heading font-semibold text-foreground flex items-center gap-2">
          <ShieldAlert className="h-4 w-4 text-primary" /> Guardrail Policies{' '}
        ...
        <Dialog open={newOpen} onOpenChange={setNewOpen}>
          ...
          <DialogContent className="sm:max-w-[420px]">
            <DialogHeader>
              <DialogTitle>Novo Guardrail</DialogTitle>
            </DialogHeader>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-[11px] text-muted-foreground mt-1">
            Crie guardrails para proteger seus agentes.
          </p>
        </div>
      ) : (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {guardrails.map((g) => (
            ...
            >
              <ShieldCheck
                className={`h-5 w-5 shrink-0 ${g.is_enabled ? 'text-nexus-emerald' : 'text-muted-foreground'}`}
              />
