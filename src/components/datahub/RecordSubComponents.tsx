...
import {
  Phone, Mail, MapPin, User, ExternalLink, Lock, Building2, MessageCircle,
  Loader2, Pencil, Check, X as XIcon, GitBranch,
} from "lucide-react";
...
export const NON_EDITABLE = new Set(['id', 'created_at', 'updated_at', 'search_vector', 'is_customer', 'is_supplier', 'is_carrier']);
      ...
      {isSens ? (
        ...
        <div className="flex items-center gap-1 flex-1 justify-end">
          <Input ref={inputRef} value={editValue} onChange={(e) => setEditValue(e.target.value)}
            onKeyDown={handleKeyDown} className="h-6 text-[11px] font-mono bg-secondary/50 border-primary/30 max-w-[250px] px-1.5" disabled={saving} />
        ...
        <div className="flex items-center gap-1 max-w-[250px]">
          ...
          {editable && (
            <button onClick={() => { setEditValue(String(value ?? '')); setEditing(true); }}
            ...
            <div className="space-y-1 max-h-[150px] overflow-y-auto">
              {rows.map((row, i) => (
                <div key={i} className="flex flex-wrap gap-x-3 gap-y-0.5 py-1 border-b border-nexus-emerald/10 last:border-0 text-[11px]">
                  {Object.entries(row as Record<string, unknown>).slice(0, 8).map(([k, v]) => (
                    <span key={k}>
                      <span className="text-muted-foreground">{k}: </span>
      ...
      <div className="space-y-1">
        {members.map((m, i) => (
          <div key={i} className="flex items-center justify-between py-1 text-[11px]">
            <span className="text-foreground">{String(m[displayCol] || m.id)}</span>
            {m.cnpj ? <span className="text-muted-foreground font-mono">{formatCNPJ(String(m.cnpj))}</span> : null}
          </div>
        ))}
      </div>
    </div>
