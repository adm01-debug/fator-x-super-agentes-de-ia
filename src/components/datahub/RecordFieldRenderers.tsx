...
import {
  Phone, Mail, MapPin, User, Building2, MessageCircle,
  ExternalLink, Lock, GitBranch, Loader2, Pencil, Check, X as XIcon,
} from "lucide-react";
...
export function FieldValue({ label, value, sensitive, editable, onSave }: {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try { await onSave?.(editValue); setEditing(false); toast.success(`Campo "${label}" atualizado`); }
    catch (e: unknown) { toast.error(`Erro ao salvar: ${e instanceof Error ? e.message : String(e)}`); }
    finally { setSaving(false); }
  };

  const handleCancel = () => { setEditValue(String(value ?? '')); setEditing(false); };
  ...
  return (
    <div className="flex justify-between items-center py-1 border-b border-border/10 last:border-0 group min-h-[28px]">
      ...
      {isSens ? (
        <span className="flex items-center gap-1 text-[11px] text-destructive/70"><Lock className="h-3 w-3" /> REDACTED (LGPD)</span>
      ) : editing ? (
        <div className="flex items-center gap-1 flex-1 justify-end">
          <Input ref={inputRef} value={editValue} onChange={(e) => setEditValue(e.target.value)} onKeyDown={handleKeyDown} className="h-6 text-[11px] font-mono bg-secondary/50 border-primary/30 max-w-[250px] px-1.5" disabled={saving} />
        ...
        <div className="flex items-center gap-1 max-w-[250px]">
          <span className="text-foreground text-[11px] font-mono text-right truncate flex-1">{value === null || value === undefined ? '—' : String(value)}</span>
          {editable && <button onClick={() => { setEditValue(String(value ?? '')); setEditing(true); }} className="opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-primary/10 text-muted-foreground hover:text-primary"><Pencil className="h-3 w-3" /></button>}
        </div>
      )}
    </div>
  );
...
export function PhoneList({ phones }: { phones: Array<Record<string, unknown>> }) {
  ...
  return (<div className="flex flex-wrap gap-2">{phones.map((p, i) => (<div key={i} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-secondary/30 border border-border/20 text-[11px]"><Phone className="h-3 w-3 text-primary" /><span className="font-mono text-foreground">{formatPhone(p.phone as string | null)}</span></div>))}</div>);
}
...
export function EmailList({ emails }: { emails: Array<Record<string, unknown>> }) {
  ...
  return (<div className="flex flex-wrap gap-2">{emails.map((e, i) => (<div key={i} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-secondary/30 border border-border/20 text-[11px]"><Mail className="h-3 w-3 text-primary" /><span className="font-mono text-foreground">{String(e.email || '')}</span></div>))}</div>);
}
...
export function AddressCard({ addresses }: { addresses: Array<Record<string, unknown>> }) {
  ...
  return (
    <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-secondary/30 border border-border/20 text-[11px]">
      ...
      <div>
        <p className="text-foreground font-medium">{[addr.cidade, addr.estado].filter(Boolean).map(String).join(', ') || '—'}</p>
    ...
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-secondary/30 border border-border/20 text-[11px]">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          {s.homologado !== undefined && <Badge variant={s.homologado ? 'default' : 'secondary'} className="text-[11px] h-4">{s.homologado ? '✅ Homologado' : 'Não homologado'}</Badge>}
        </div>
        {s.score_geral !== undefined && <p className="text-muted-foreground">Score: <span className="text-foreground font-mono">{String(s.score_geral)}</span></p>}
        {s.data_homologacao ? <p className="text-muted-foreground">Homologado em: {formatDate(s.data_homologacao as string)}</p> : null}
      </div>
    </div>
  );
}
...
export function MessagesTimeline({ messages }: { messages: Array<Record<string, unknown>> }) {
  ...
  return (
    <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
      ...
      {messages.length > 20 && <p className="text-[11px] text-muted-foreground text-center">+{messages.length - 20} mensagens...</p>}
    </div>
  );
}

export function GenericSecondaryCard({ tableName, data }: { tableName: string; data: Array<Record<string, unknown>> }) {
  if (!data?.length) return null;
  return (
        ...
        {data.map((row, i) => (
          <div key={i} className="flex flex-wrap gap-x-3 gap-y-0.5 py-1 border-b border-border/10 last:border-0">
            {Object.entries(row).map(([k, v]) => (
              <span key={k} className="text-[11px]"><span className="text-muted-foreground">{k}: </span><span className="text-foreground font-mono">{v === null ? '—' : String(v)}</span></span>
            ))}
          </div>
        ))}
      </div>
        ...
        return (
          <div key={key} className="rounded-lg bg-nexus-emerald/5 border border-nexus-emerald/20 p-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge variant="outline" className="text-[11px] h-4 border-nexus-emerald/30 text-nexus-emerald">{rows.length} resultado{rows.length !== 1 ? 's' : ''}</Badge>
            </div>
              ...
              {(rows as Record<string, unknown>[]).map((row, i) => (
                <div key={i} className="flex flex-wrap gap-x-3 gap-y-0.5 py-1 border-b border-nexus-emerald/10 last:border-0 text-[11px]">
                  {Object.entries(row).slice(0, 8).map(([k, v]) => (
                    <span key={k}><span className="text-muted-foreground">{k}: </span><span className="text-foreground font-mono">{v === null ? '—' : String(v).slice(0, 50)}</span></span>
                  ))}
                </div>
              ))}
            </div>
...
export function GroupMembers({ entityId, grupoId, excludeId }: { entityId: string; grupoId: string; excludeId: string }) {
  ...
  const { ENTITY_MAPPINGS } = require("@/config/datahub-entities");
