...
interface DataBrowserFiltersProps {
  ...
  newFilterCol: string;
  newFilterVal: string;
  onNewFilterColChange: (col: string) => void;
  onNewFilterValChange: (val: string) => void;
  onAddFilter: () => void;
  onRemoveFilter: (index: number) => void;
  onClearFilters: () => void;
}

export function DataBrowserFilters({
  filterOptions, filters, newFilterCol, newFilterVal,
  onNewFilterColChange, onNewFilterValChange,
  onAddFilter, onRemoveFilter, onClearFilters,
  ...
  return (
    <div className="rounded-lg bg-secondary/20 border border-border/30 p-3 space-y-3 animate-in slide-in-from-top-1 duration-150">
      <div className="flex items-center gap-2 flex-wrap">
        <Select value={newFilterCol} onValueChange={onNewFilterColChange}>
        ...
        {newFilterCol && (() => {
          ...
          if (opt?.type === 'select') {
            return (
              <Select value={newFilterVal} onValueChange={onNewFilterValChange}>
        ...
        <Button size="sm" variant="default" className="h-8 gap-1 text-xs" onClick={onAddFilter} disabled={!newFilterCol || !newFilterVal}>
