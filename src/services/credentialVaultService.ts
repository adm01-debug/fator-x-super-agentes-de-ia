...
async function getOrCreateMasterKey(): Promise<CryptoKey> {
  ...
  const keyMaterial = await crypto.subtle.importKey('raw', seed, { name: 'PBKDF2' }, false, [
    'deriveKey',
  ]);
...
export async function getCredential(
  ...
  if (
    requesterType === 'agent' &&
    requesterId &&
    entry.allowed_agents.length > 0 &&
    !entry.allowed_agents.includes(requesterId)
  )
    throw new Error('Agent not authorized');
  if (
    requesterType === 'workflow' &&
    requesterId &&
    entry.allowed_workflows.length > 0 &&
    !entry.allowed_workflows.includes(requesterId)
  )
    throw new Error('Workflow not authorized');
  ...
  await fromTable('credential_vault')
    .update({ access_count: entry.access_count + 1, last_accessed_at: new Date().toISOString() })
    .eq('id', id);
...
export async function listCredentials(filters?: {
  ...
  let query = fromTable('credential_vault')
    .select(
      'id, name, description, credential_type, service_name, status, expires_at, rotation_interval_days, last_rotated_at, next_rotation_at, allowed_agents, allowed_workflows, access_count, last_accessed_at, tags, created_by, created_at, updated_at',
    )
...
export async function rotateCredential(
  ...
  let nextRotation: string | null = null;
  const interval = (existing.data as Record<string, unknown>).rotation_interval_days as
    | number
    | null;
  if (interval) {
...
export async function getVaultStats(): Promise<VaultStats> {
  ...
  for (const c of creds) {
    ...
    if (c.expires_at && new Date(c.expires_at) <= sevenDays && c.status === 'active')
      expiringSoon++;
