...
export async function createSchedule(input: CreateScheduleInput): Promise<CronSchedule> {
  ...
  if (input.frequency === 'once' && input.start_at) next_run_at = input.start_at;
    ...
    const n = new Date();
    n.setSeconds(n.getSeconds() + input.interval_seconds);
    next_run_at = n.toISOString();
  ...
  if (status === 'success' || status === 'failed') {
    ...
    if (schedule) {
      ...
      if (shouldComplete) {
        ...
        updatePayload.next_run_at = getNextCronRun(schedule.cron_expression).toISOString();
      else if (schedule.interval_seconds) {
        const n = new Date();
        n.setSeconds(n.getSeconds() + schedule.interval_seconds);
        updatePayload.next_run_at = n.toISOString();
      }
...
export async function getScheduleStats(): Promise<ScheduleStats> {
  ...
  const durations = execs.map((e) => e.duration_ms).filter((d): d is number => d !== null);
