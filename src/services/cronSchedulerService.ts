...
export async function getScheduleStats(): Promise<ScheduleStats> {
  ...
  const active = allSchedules.filter((s: CronSchedule) => s.status === 'active');
  const paused = allSchedules.filter((s: CronSchedule) => s.status === 'paused');
  ...
  const successCount = execs.filter((e: ScheduleExecution) => e.status === 'success').length;
  const durations = execs
    .map((e: ScheduleExecution) => e.duration_ms)
    .filter((d): d is number => d !== null);

  const upcoming = active
    .filter((s: CronSchedule) => s.next_run_at !== null)
    .sort(
      (a: CronSchedule, b: CronSchedule) =>
