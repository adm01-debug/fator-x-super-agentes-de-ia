...
>[] = [
  ...
  },
  {
    name: 'Slack',
    slug: 'slack',
    description: 'Mensagens, canais, reações, threads — comunicação do time',
    icon: '💬',
    color: '#4A154B',
    category: 'communication',
    auth_type: 'bearer',
    auth_config_schema: { bot_token: 'string', signing_secret: 'string' },
    base_url: 'https://slack.com/api',
    api_version: 'v1',
    operations: [
      { id: 'send-message', name: 'Enviar Mensagem', description: 'Envia mensagem para canal ou DM', type: 'action', input_schema: { channel: 'string', text: 'string' }, output_schema: { ts: 'string' } },
      { id: 'message-received', name: 'Mensagem Recebida', description: 'Trigger ao receber mensagem', type: 'trigger', input_schema: {}, output_schema: { channel: 'string', text: 'string', user: 'string' } },
      { id: 'add-reaction', name: 'Adicionar Reação', description: 'Adiciona emoji a mensagem', type: 'action', input_schema: { channel: 'string', timestamp: 'string', name: 'string' }, output_schema: { ok: 'boolean' } },
    ],
    rate_limit_per_minute: 60,
    supports_webhooks: true,
    supports_polling: false,
    ...
    status: 'available',
    version: '1.0.0',
    tags: ['slack', 'mensagem', 'time', 'comunicacao'],
  },
  {
    name: 'Google Sheets',
    slug: 'google-sheets',
    description: 'Leitura e escrita em planilhas Google — relatórios, dados, exports',
    icon: '📊',
    color: '#0F9D58',
    category: 'productivity',
    auth_type: 'oauth2',
    auth_config_schema: { client_id: 'string', client_secret: 'string', refresh_token: 'string' },
    base_url: 'https://sheets.googleapis.com/v4',
    api_version: 'v4',
    operations: [
      { id: 'read-range', name: 'Ler Range', description: 'Lê dados de um range da planilha', type: 'action', input_schema: { spreadsheetId: 'string', range: 'string' }, output_schema: { values: 'array' } },
      { id: 'append-row', name: 'Adicionar Linha', description: 'Adiciona linha ao final da planilha', type: 'action', input_schema: { spreadsheetId: 'string', range: 'string', values: 'array' }, output_schema: { updates: 'object' } },
      { id: 'update-range', name: 'Atualizar Range', description: 'Atualiza dados em um range', type: 'action', input_schema: { spreadsheetId: 'string', range: 'string', values: 'array' }, output_schema: { updatedCells: 'number' } },
    ],
    rate_limit_per_minute: 60,
    supports_webhooks: false,
    supports_polling: true,
    ...
    status: 'available',
    version: '1.0.0',
    tags: ['google', 'planilha', 'dados', 'relatorio'],
  },
];
