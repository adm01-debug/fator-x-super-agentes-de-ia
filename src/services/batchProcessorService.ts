...
export async function reportBatchResults(
  ...
  if (job.error_policy === 'threshold') {
    if ((newFailed / newProcessed) * 100 > job.error_threshold_pct) {
      finalStatus = 'failed';
      shouldContinue = false;
    }
  }
  ...
  for (let batch = 0; batch < totalBatches; batch++) {
    ...
    if (options.onProgress) {
      ...
      options.onProgress({
        ...
        total_batches: totalBatches,
        elapsed_ms: elapsed,
        estimated_remaining_ms: ips > 0 ? (remaining / ips) * 1000 : 0,
        items_per_second: Math.round(ips * 10) / 10,
      });
    }
...
export async function getBatchStats(): Promise<{
  ...
  const { data, error } = await fromTable('batch_jobs').select(
    'status, processed_items, successful_items, failed_items, duration_ms, avg_item_ms',
  );
  if (error) throw error;
  const jobs: Record<string, unknown>[] = data ?? [];
  const completed = jobs.filter((j) => j.status === 'completed' || j.status === 'partial');
  const failed = jobs.filter((j) => j.status === 'failed');
  const running = jobs.filter((j) => j.status === 'running');
  const totalProcessed = jobs.reduce(
    (s: number, j: Record<string, unknown>) => s + (j.processed_items ?? 0),
    0,
  );
  const totalSuccessful = jobs.reduce(
    (s: number, j: Record<string, unknown>) => s + (j.successful_items ?? 0),
    0,
  );
  ...
  return {
    ...
    avg_items_per_second:
      speeds.length > 0 ? 1000 / (speeds.reduce((a, b) => a + b, 0) / speeds.length) : 0,
  };
}
