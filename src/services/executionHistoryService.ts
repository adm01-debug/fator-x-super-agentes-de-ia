...
export function compareExecutions(
  ...
  const stepDiffs = a.steps.map((stepA) => {
    const stepB = b.steps.find((s: ExecutionStepRecord) => s.step_name === stepA.step_name);
    return {
    ...
    .map(([hour, b]) => ({
      ...
      avg_duration_ms:
        b.durations.length > 0
          ? b.durations.reduce((sum: number, v: number) => sum + v, 0) / b.durations.length
          : 0,
    }));
}
...
export async function getExecutionStats(
  ...
  const items = data ?? [];
  const success = items.filter((i: ExecutionRecord) => i.status === 'success');
  const failed = items.filter((i: ExecutionRecord) => i.status === 'failed');
  const running = items.filter((i: ExecutionRecord) => i.status === 'running');
  const durations = items.map((i: ExecutionRecord) => i.duration_ms).filter((d): d is number => d !== null);
  ...
  return {
    ...
    avg_duration_ms: durations.length > 0 ? durations.reduce((sum: number, d: number) => sum + d, 0) / durations.length : 0,
    total_tokens: items.reduce((s: number, i: ExecutionRecord) => s + (i.tokens_used ?? 0), 0),
    total_cost_brl: items.reduce((s: number, i: ExecutionRecord) => s + (i.cost_brl ?? 0), 0),
    by_type: byType,
  };
}
