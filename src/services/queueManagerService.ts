...
export async function getQueueMetrics(queueId: string): Promise<QueueMetrics> {
  ...
  const all = items ?? [];
  const now = Date.now();
  const pending = all.filter((i: QueueItem) => i.status === 'pending');
  const processing = all.filter((i: QueueItem) => i.status === 'processing');
  const completed = all.filter((i: QueueItem) => i.status === 'completed');
  const failed = all.filter((i: QueueItem) => i.status === 'failed');
  const deadLetter = all.filter((i: QueueItem) => i.status === 'dead_letter');

  // Throughput: completed items in last minute
  const oneMinuteAgo = new Date(now - 60000).toISOString();
  const recentCompleted = completed.filter((i: QueueItem) => i.completed_at && i.completed_at >= oneMinuteAgo);

  // Wait time
  const waitTimes = processing
    .filter((i: QueueItem) => i.created_at && i.started_at)
    .map((i: QueueItem) => new Date(i.started_at!).getTime() - new Date(i.created_at).getTime());

  // Processing time
  const processingTimes = completed
    .filter((i: QueueItem) => i.started_at && i.completed_at)
    .map((i: QueueItem) => new Date(i.completed_at!).getTime() - new Date(i.started_at!).getTime());
  ...
  return {
    ...
    avg_wait_time_ms: waitTimes.length > 0 ? waitTimes.reduce((sum: number, t: number) => sum + t, 0) / waitTimes.length : 0,
    avg_processing_time_ms: processingTimes.length > 0 ? processingTimes.reduce((sum: number, t: number) => sum + t, 0) / processingTimes.length : 0,
