...
export function calculateDelay(attempt: number, policy: RetryPolicy): number {
  ...
  switch (policy.backoff_strategy) {
    ...
    default: {
      ...
      delay = base + Math.random() * base * 0.3;
      break;
    }
  }
...
function isRetryable(error: string, policy: RetryPolicy): boolean {
  if (policy.non_retryable_errors.some((e) => error.includes(e))) return false;
...
export function canExecute(serviceName: string): boolean {
  ...
  if (cb.state === 'open') {
    if (Date.now() - new Date(cb.last_state_change_at).getTime() >= cb.config.timeout_ms) {
      cb.state = 'half_open';
      cb.success_count = 0;
      cb.failure_count = 0;
      cb.last_state_change_at = new Date().toISOString();
...
export async function executeWithCircuitBreaker<T>(
  ...
  if (result.success) recordCircuitSuccess(serviceName);
  else recordCircuitFailure(serviceName);
  return result;
}
