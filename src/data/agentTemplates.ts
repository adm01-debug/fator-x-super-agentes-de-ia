...
export function templateToConfig(template: AgentTemplate): AgentConfig {
  return {
    ...
    persona: (template.type === 'sdr' ? 'specialist' : template.type === 'support' ? 'assistant' : template.type === 'orchestrator' ? 'coordinator' : template.type === 'analyst' ? 'analyst' : 'assistant') as AgentConfig['persona'],
