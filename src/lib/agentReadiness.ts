...
export function computeReadinessScore(agent: AgentConfig): ReadinessResult {
  const checks: ReadinessCheck[] = [
    { id: 'name', label: 'Nome do agente definido', passed: !!agent.name?.trim(), tabId: 'identity' },
    { id: 'mission', label: 'Missão descrita', passed: !!agent.mission && agent.mission.length >= 20, tabId: 'identity' },
    ...
    { id: 'system_prompt', label: 'System prompt ≥ 200 caracteres', passed: (agent.system_prompt?.length ?? 0) >= 200, tabId: 'prompt' },
    { id: 'tools', label: '≥ 1 ferramenta ativa', passed: (agent.tools ?? []).some((t: any) => t.enabled), tabId: 'tools' },
...
export const READINESS_COLORS: Record<ReadinessResult['level'], { bg: string; text: string; border: string; label: string }> = {
  critical: { bg: 'bg-destructive/10', text: 'text-destructive', border: 'border-destructive/40', label: 'Crítico' },
  warning: { bg: 'bg-nexus-amber/10', text: 'text-nexus-amber', border: 'border-nexus-amber/40', label: 'Em progresso' },
  good: { bg: 'bg-nexus-emerald/10', text: 'text-nexus-emerald', border: 'border-nexus-emerald/40', label: 'Quase lá' },
  excellent: { bg: 'bg-nexus-emerald/15', text: 'text-nexus-emerald', border: 'border-nexus-emerald/60', label: 'Pronto' },
};
